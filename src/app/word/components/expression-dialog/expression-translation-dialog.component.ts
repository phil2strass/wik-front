...
import { WordExpressionTranslation } from '../../models/expression.model';
...
@Component({
    selector: 'app-expression-translation-dialog',
    standalone: true,
    templateUrl: './expression-translation-dialog.component.html',
    styleUrls: ['../example-dialog/example-dialog.component.scss', '../example-dialog/example-translation-dialog.component.scss'],
...
export class ExpressionTranslationDialogComponent {
    ...
    constructor(
        @Inject(MAT_DIALOG_DATA) public data: ExpressionTranslationDialogData,
        private dialogRef: MatDialogRef<ExpressionTranslationDialogComponent>
    ) {
    ...
    private buildGroup(translation: WordExpressionTranslation): FormGroup {
        return this.#fb.group({
            translationId: [translation.translationId ?? null],
            expressionId: [translation.expressionId],
            expressionContent: [translation.expressionContent],
    ...
    deleteTranslation(index: number): void {
        ...
        const expressionId = group.get('expressionId')?.value;
        if (!expressionId) {
            return;
        }
        this.loading = true;
        this.#expressionService
            .deleteExpressionTranslation(expressionId, this.data.langue.id)
            .pipe(finalize(() => (this.loading = false)))
            .subscribe({
                next: () => {
                    group.patchValue({ translationId: null, content: '' });
                    this.editing.delete(index);
                    this.#messageService.info(this.#translate.instant('word.expressions.translation.deleted'));
                },
                error: err => {
    ...
    saveAll(): void {
        const payload: WordExpressionTranslation[] = [];
            ...
            .subscribe({
                next: saved => {
                    saved.forEach(savedItem => {
                        const idx = this.forms.controls.findIndex(ctrl => ctrl.get('expressionId')?.value === savedItem.expressionId);
                        if (idx >= 0) {
                    ...
                    this.#messageService.info(this.#translate.instant('word.expressions.translation.saved'));
                    this.editing.clear();
                },
                error: err => {
