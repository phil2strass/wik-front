...
type ExpressionDeleteDialogData = {
    content: string;
};

@Component({
    selector: 'app-expression-delete-dialog',
    standalone: true,
    template: `
        ...
        <div mat-dialog-actions class="d-flex justify-content-end gap-12">
            ...
            <button color="warn" mat-flat-button [mat-dialog-close]="true">
                {{ 'word.expressions.delete' | translate }}
            </button>
        </div>
    `,
    styles: [
...
export class ExpressionDeleteDialogComponent {
    readonly data = inject<ExpressionDeleteDialogData>(MAT_DIALOG_DATA);
    readonly dialogRef = inject(MatDialogRef<ExpressionDeleteDialogComponent>);
